...
icon = Image.open(r"C:\Users\shali\Desktop\python file\PhonePe-Emblem.png")
st.set_page_config(page_title= "Phonepe Pulse Data Visualization | By Archana",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This dashboard app is created by *Archana*!
                                        Data has been cloned from Phonepe Pulse Github Repo"""})

st.sidebar.image(Image.open("PhonePe-Emblem.png"))
...
conn = pymysql.connect(host='127.0.0.1', user='root', password='PHW#84#jeor',port=3306, db='phonepe_pulse')
cursor = conn.cursor()
...
# Top Charts - TRANSACTIONS    
    if Type == "Transactions":
        ...
        with col2:
            ...
            cursor.execute(f"select district , sum(Transaction_count) as Total_Count, sum(Transaction_Amount) as Total from map_transaction where year = {Year} and quarter = {Quarter} group by district order by Total desc limit 10")
        ...
        with col3:
            ...
            cursor.execute(f"select District_Pincode, sum(Transaction_count) as Total_Transactions_Count, sum(Transaction_amount) as Total from top_transaction where year = {Year} and quarter = {Quarter} group by District_Pincode order by Total desc limit 10")
    ...
    if Type == "Users":
        ...
        with col1:
            ...
            if Year == 2023 and Quarter in [4]:
                st.markdown("#### Sorry No Data to Display for 2022 Qtr 4")
            else:
                cursor.execute(f"select Brands, sum(User_count) as Total_Count, avg(User_percentage)*100 as Avg_Percentage from aggregated_user where year = {Year} and quarter = {Quarter} group by brands order by Total_Count desc limit 10")
        ...
        with col2:
            st.markdown("### :violet[District]")
            cursor.execute(f"select district, sum(Registered_User) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by district order by Total_Users desc limit 10")
        ...
        with col3:
            st.markdown("### :violet[Pincode]")
            cursor.execute(f"select District_Pincode, sum(Registered_User) as Total_Users from top_user where year = {Year} and quarter = {Quarter} group by District_Pincode order by Total_Users desc limit 10")
        ...
        with col4:
            st.markdown("### :violet[State]")
            cursor.execute(f"select state, sum(Registered_user) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by Total_Users desc limit 10")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            df2 = pd.read_csv(r'C:\Users\shali\Desktop\python file\Statenames.csv')
            df1.State = df2
...
# EXPLORE DATA - USERS      
    if Type == "Users":
        ...
        cursor.execute(f"select state, sum(Registered_User) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by state")
        ...
        cursor.execute(f"select State,year,quarter,District,sum(Registered_user) as Total_Users, sum(AppOpens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} and state = '{selected_state}' group by State, District,year,quarter order by state,district")
